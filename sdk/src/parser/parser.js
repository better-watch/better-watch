...
export class TypeScriptParser {
    ...
    parse(sourceCode, options = {}) {
        const { isModule = true, isTypeScript = this.detectTypeScript(sourceCode), hasJSX = this.detectJSX(sourceCode), filename = 'unknown.ts', sourceMap = true, } = options;
        const parserOptions = this.buildParserOptions(isModule, isTypeScript, hasJSX, filename, sourceMap);
        const errors = [];
        let ast = null;
        let tokens = [];
        try {
     ...
     */
    parseModule(sourceCode, filename) {
     ...
     */
    parseScript(sourceCode, filename) {
        return this.parse(sourceCode, {
            isModule: false,
            filename,
        });
     ...
     */
    buildParserOptions(isModule, isTypeScript, hasJSX, filename, sourceMap) {
     ...
     */
    extractSourceMap(sourceCode, _ast) {
