...
import type { VitePluginConfig } from './types.js';

/**
 * Default configuration for Vite plugin
 */
export const DEFAULT_VITE_CONFIG: VitePluginConfig = {
  enabled: true,
  trace: {
    functions: ['*'],
    classes: ['*'],
...
/**
 * Validate Vite plugin configuration
 *
 * @param config - Configuration to validate
 * @returns Validation result with errors if any
 */
export function validateViteConfig(config: unknown): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function mergeConfig(config?: VitePluginConfig): VitePluginConfig {
  return {
    enabled:
      config?.enabled !== undefined ? config.enabled : DEFAULT_VITE_CONFIG.enabled,
    trace: {
      functions:
        config?.trace?.functions !== undefined
          ? config.trace.functions
          : DEFAULT_VITE_CONFIG.trace?.functions || [],
      classes:
        config?.trace?.classes !== undefined
          ? config.trace.classes
          : DEFAULT_VITE_CONFIG.trace?.classes || [],
      lines:
        config?.trace?.lines !== undefined
          ? config.trace.lines
          : DEFAULT_VITE_CONFIG.trace?.lines || [],
    },
    performance: {
      samplingRate:
        config?.performance?.samplingRate !== undefined
          ? config.performance.samplingRate
          : DEFAULT_VITE_CONFIG.performance?.samplingRate || 1.0,
      timeoutMs:
        config?.performance?.timeoutMs !== undefined
          ? config.performance.timeoutMs
          : DEFAULT_VITE_CONFIG.performance?.timeoutMs || 5000,
    },
  };
}
