...
import type { RollupPluginConfig } from './types.js';

/**
 * Default configuration for Rollup plugin
 */
export const DEFAULT_ROLLUP_CONFIG: RollupPluginConfig = {
  enabled: true,
  trace: {
    functions: ['*'],
    classes: ['*'],
...
/**
 * Validate Rollup plugin configuration
 *
 * @param config - Configuration to validate
 * @returns Validation result with errors if any
 */
export function validateRollupConfig(config: unknown): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function mergeConfig(config?: RollupPluginConfig): RollupPluginConfig {
  return {
    enabled:
      config?.enabled !== undefined ? config.enabled : DEFAULT_ROLLUP_CONFIG.enabled,
    trace: {
      functions:
        config?.trace?.functions !== undefined
          ? config.trace.functions
          : DEFAULT_ROLLUP_CONFIG.trace?.functions || [],
      classes:
        config?.trace?.classes !== undefined
          ? config.trace.classes
          : DEFAULT_ROLLUP_CONFIG.trace?.classes || [],
      lines:
        config?.trace?.lines !== undefined
          ? config.trace.lines
          : DEFAULT_ROLLUP_CONFIG.trace?.lines || [],
    },
    performance: {
      samplingRate:
        config?.performance?.samplingRate !== undefined
          ? config.performance.samplingRate
          : DEFAULT_ROLLUP_CONFIG.performance?.samplingRate || 1.0,
      timeoutMs:
        config?.performance?.timeoutMs !== undefined
          ? config.performance.timeoutMs
          : DEFAULT_ROLLUP_CONFIG.performance?.timeoutMs || 5000,
    },
  };
}
