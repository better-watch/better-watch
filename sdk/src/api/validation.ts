 ...
 */
export function validateTraceEvent(trace: unknown): ValidationError[] {
  ...
  if (
    !t.type ||
    !['before', 'after', 'entry', 'exit'].includes(t.type as string)
  ) {
    errors.push({
      field: 'type',
  ...
  if (!t.timestamp || typeof t.timestamp !== 'string') {
    ...
    const date = new Date(t.timestamp as string);
 ...
 */
export function validateTracesInBatch(
  request: TraceBatchRequest
): Array<{ index: number; errors: ValidationError[] }> {
  const results: Array<{ index: number; errors: ValidationError[] }> = [];
