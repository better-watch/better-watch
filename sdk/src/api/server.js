...
import { InMemoryStorage, ClickHouseStorage, PostgreSQLStorage, } from './storage.js';
/**
 * Trace Ingestion API Server
 */
export class TraceIngestionServer {
    ...
    initializeStorage() {
        switch (this.config.storage.type) {
            case 'clickhouse':
                return new ClickHouseStorage(this.config.storage.connection || 'http://localhost:8123');
            case 'postgresql':
                return new PostgreSQLStorage(this.config.storage.connection ||
     ...
     */
    async ingestTraces(payload, payloadSize) {
        this.requestCounter++;
        ...
        // Authenticate
        const isAuthenticated = await this.authenticator.authenticate(request.apiKey, request.projectId);
        if (!isAuthenticated) {
            return {
                success: false,
        ...
        if (traceValidationErrors.length > 0) {
            const acceptedTraces = request.traces.filter((_, index) => !traceValidationErrors.some((e) => e.index === index));
            // Store valid traces
            let acceptedCount = 0;
