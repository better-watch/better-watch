...
import type { EsbuildPluginConfig } from './types.js';

/**
 * Default configuration for esbuild plugin
 */
export const DEFAULT_ESBUILD_CONFIG: EsbuildPluginConfig = {
  enabled: true,
  trace: {
    functions: ['*'],
    classes: ['*'],
...
/**
 * Validate esbuild plugin configuration
 *
 * @param config - Configuration to validate
 * @returns Validation result with errors if any
 */
export function validateEsbuildConfig(config: unknown): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
 ...
 */
export function mergeConfig(config?: EsbuildPluginConfig): EsbuildPluginConfig {
  return {
    enabled:
      config?.enabled !== undefined ? config.enabled : DEFAULT_ESBUILD_CONFIG.enabled,
    trace: {
      functions:
        config?.trace?.functions !== undefined
          ? config.trace.functions
          : DEFAULT_ESBUILD_CONFIG.trace?.functions || [],
      classes:
        config?.trace?.classes !== undefined
          ? config.trace.classes
          : DEFAULT_ESBUILD_CONFIG.trace?.classes || [],
      lines:
        config?.trace?.lines !== undefined
          ? config.trace.lines
          : DEFAULT_ESBUILD_CONFIG.trace?.lines || [],
    },
    performance: {
      samplingRate:
        config?.performance?.samplingRate !== undefined
          ? config.performance.samplingRate
          : DEFAULT_ESBUILD_CONFIG.performance?.samplingRate || 1.0,
      timeoutMs:
        config?.performance?.timeoutMs !== undefined
          ? config.performance.timeoutMs
          : DEFAULT_ESBUILD_CONFIG.performance?.timeoutMs || 5000,
    },
  };
}
